...
set C_backload default {};
set C_max;
set C_equal default C;
set C_slop default C;

set T2{c in C} within T default {}; # for partial discharge
set T3{c in C} within T default {}; # for partial discharge slop tank
...
set P_load{c in C} default 1 .. LP ; # set of loading ports
set P_load2{c in C} default {} ; # set of discharging ports for discharge module
set P_load3{c in C} default {} ; # set of discharging ports for discharge module slop tank
set P_load4{c in C} default {} ; # set of discharging ports for discharge module slop tank
set P_dis{c in C} default LP+1 .. NP ; # set of discharging ports
...
## discharge
set manual within C cross T cross P default {}; 
set balance within C cross T cross P default {}; 
set remain within C cross T cross P default {}; 
set manual1 within C cross P default {}; 
set balance1 within C  cross P default {}; 
set remain1 within C cross P default {}; 

param sounding4mVol{t in T} default 0;
param vol30percent{t in T}  default 0;
...
### Decision Variables ###
# for compatibility
var x1{C,T,Pbar} binary; # 1 if cargo c is allocated into tank t
...
var zw{C,T,P} binary; # for min discharging amount from each tank
var zqw{C,T,P} binary; # for min amount remaining in each tank
...
var delta{c in C diff C_locked, p in P_dis[c]} binary;
...
sum{(c,t,p) in manual union remain} -priority[c]*w[c,t,p]
+ sum{(c,t,p) in balance} priority[c]*w[c,t,p]
-1*sum{t in TB, p in P}wB[t,p]
- maxEmptyTankWeight*sum{ (c, p) in P_opt, t in T} qwz[c,t,p]
+sum{c in C_backload, t in Tc[c]} priority[c]*(sum{p in P_load[c]} qw[c,t,p]- sum{p in P_dis[c]} w[c,t,p]);

#- 100000*sum{c in C, p in P} qwx[c,p];
...
### Constraints ###
## basic conditions
subject to Condition0 {c in C diff C_loaded diff C_locked, p in P}: sum{t in T diff Tc[c]} x1[c,t,p] = 0; # cargo c is not allocated to tank t if t is not compatible with cargo c
subject to Condition01 {t in T diff Tm, p in P}: sum{k in C} x1[k,t,p] <= 1; # one tank can only take in one cargo
subject to Condition01f1 {c in C, t in T}: sum{p in P} x1[c,t,p] <= 100*x[c,t]; # link x and x1
subject to Condition01f2 {c in C, t in T}: sum{p in P} x1[c,t,p] >= x[c,t]; # link x and x1
...
subject to Condition01z {c in C_backload, p in P_load[c]}: sum{t in Tc[c]} qw[c,t,p] >= minCargoLoad[c]; # a cargo is loaded to at min amt at last loading port

subject to Condition020 {c in C, t in T diff Tc[c], p in P}: y[c,t,p] = x1[c,t,p]; # cargo c is not allocated to tank t if t is not compatible with cargo c
subject to Condition021 {c in C, t in T diff Tc[c], p in P}: qty[c,t,p] = x1[c,t,p]; # cargo c is not allocated to tank t if t is not compatible with cargo c
subject to Condition03 {c in C_backload}: sum{t in Tc[c]} Q0[c,t] + sum{p in P_load[c], t in Tc[c]} y[c,t,p] >= sum{p in P_dis[c], t in Tc[c]} -y[c,t,p]; # total loaded >= total discharged

subject to Condition04 {c in C_backload, p in P_load[c] : Vcp[c,p]>0}:  0 <= sum{t in Tc[c]} y[c,t,p] <= Vcp[c,p]; # amount of cargo to be loaded <= amount available at the port.
subject to Condition041 {c in C_backload, t in Tc[c], p in P_load[c]}: y[c,t,p]>= 0;

subject to Condition05 {c in C diff  C_locked, p in P_dis[c] : Vcp[c,p]<0}:  0 >= sum{t in Tc[c]} y[c,t,p] >= Vcp[c,p]; # amount of cargo to be discharged is not more than the amount needed at the port.
## if delta = 0, -inf <=  qty[p-1] <= -Vcp[p]
## if delta = 1, -Vcp[p] <=  qty[p-1] <= inf
subject to Condition050 {(c,p) in manual1 union remain1  : Vcp[c,p]<0}:  sum{t in Tc[c]}qty[c,t,p-1]>=-Vcp[c,p]-1e6*(1-delta[c,p]);
subject to Condition050a {(c,p) in manual1 union remain1 : Vcp[c,p]<0}:  sum{t in Tc[c]}qty[c,t,p-1]<=-Vcp[c,p]+1e6*delta[c,p];
## if delta = 0,  -inf <= y[p] <= inf
## if delta = 1,   -Vcp[p] <= y[p] <= -Vcp[p]
subject to Condition050b {(c,p) in manual1 union remain1  : Vcp[c,p]<0}:  -Vcp[c,p]-1e6*(1-delta[c,p])<=-sum{t in Tc[c]} y[c,t,p];
subject to Condition050b1 {(c,p) in manual1 union remain1  : Vcp[c,p]<0}:  -sum{t in Tc[c]} y[c,t,p]<=-Vcp[c,p]+1e6*(1-delta[c,p]);
## if delta = 0,   0 <= qty[p] <= 0
## if delta = 1,   -inf <= qty[p] <= Inf
subject to Condition050c {(c,p) in manual1 union remain1  : Vcp[c,p]<0}:  -1e6*(delta[c,p])<=-sum{t in Tc[c]} qty[c,t,p];
subject to Condition050c1 {(c,p) in manual1 union remain1  : Vcp[c,p]<0}:  -sum{t in Tc[c]} qty[c,t,p]<=1e6*(delta[c,p]);


subject to Condition051 {c in C diff C_backload, t in Tc[c], p in P_dis[c]}: y[c,t,p]<=0;
subject to Condition051a {c in C, t in Tc[c], p in P_dis[c]}: y[c,t,p]<=0;
...
## preloaded condition
subject to condition22 {c in C_loaded, t in T_loaded}: x1[c,t,0] = I_loaded[c,t]; # follow the existing stowage of preloaded cargoes
subject to condition23 {c in C_loaded, t in T diff T_loaded, p in P}: x1[c,t,p] = 0; # preloaded cargo can only be loaded to its corresponding preloaded tanks.
subject to condition23a {c in C_loaded, t in T diff T_loaded, p in P}: y[c,t,p] = x1[c,t,p]; # preloaded cargo can only be loaded to its corresponding preloaded tanks.
subject to condition23b {c in C_loaded, t in T diff T_loaded, p in P}: qty[c,t,p] = x1[c,t,p]; # preloaded cargo can only be loaded to its corresponding preloaded tanks.
...
subject to condition25 {c in C_locked, t in T_locked}: x1[c,t,0] = A_locked[c,t]; # follow the existing stowage of locked cargoes
subject to condition26 {c in C_locked, t in T diff T_locked, p in P}: x1[c,t,p] = 0; # locked cargo can only be loaded to its corresponding locked tanks.
subject to condition26a {c in C_locked, t in T diff T_locked, p in P}: y[c,t,p] = x1[c,t,p]; # locked cargo can only be loaded to its corresponding locked tanks.
subject to condition26b {c in C_locked, t in T diff T_locked, p in P}: qty[c,t,p] = x1[c,t,p]; # locked cargo can only be loaded to its corresponding locked tanks.
...
subject to Constr5a {c in C_backload, t in Tc[c], p in P_load[c]}: qty[c,t,p] <= upperBoundC[c,t]*capacityCargoTank[t]*x1[c,t,p];
...
subject to Constr8 {c in C_backload, t in Tc[c], p in P_load[c]}: qw2f[c,t,p]/1000 - 1e4*x1[c,t,p] <= 0; ## newly added on 20190603
...
subject to Constr12a1 {t in T}: sum {c in C_backload, p in P} x1[c,t,p] <= 100*xt[t];
...
## symmetric loading
subject to Condition112a {c in C, p in P}: x1[c,'1P',p] = x1[c,'1S',p];
subject to Condition112b {c in C, p in P}: x1[c,'2P',p] = x1[c,'2S',p];
subject to Condition112c1 {c in C, p in P}: x1[c,'3P',p] = x1[c,'3S',p];
subject to Condition112c2 {c in C, p in P}: x1[c,'4P',p] = x1[c,'4S',p];
subject to Condition112c3 {c in C, p in P}: x1[c,'5P',p] = x1[c,'5S',p];

subject to Condition112a1 {c in C_backload, (u,v) in symmetricVolTank, p in P_load[c]}: qw[c,u,p]/densityCargo_Low[c]/capacityCargoTank[u] - qw[c,v,p]/densityCargo_Low[c]/capacityCargoTank[v] <= diffVol ;
subject to Condition112a2 {c in C_backload, (u,v) in symmetricVolTank, p in P_load[c]}:     -diffVol <= qw[c,u,p]/densityCargo_Low[c]/capacityCargoTank[u] - qw[c,v,p]/densityCargo_Low[c]/capacityCargoTank[v];
...
#subject to Condition112f {c in C, (u,v) in cargoTankNonSym, p in P}: x[c,u,p] + x[c,v,p] <= diffSlop;
...
##
subject to Condition117a {c in C, t in QWT[c], p in P_load[c]}:  0.98*QW[c,t] <= qw[c,t,p] <= QW[c,t]*1.02;
subject to Condition117b {c in C, t in QWT1[c], p in dischargePort}:  0.98*QW1[c,t,p] <= qw[c,t,p] <= QW1[c,t,p]*1.02;

## 
subject to Condition112b1 {t in T, c in C diff C_loaded diff C_locked, p in P_load[c]}: qw[c,t,p] >= minCargoAmt*x1[c,t,p]; # link xB and wB
## subject to Condition112b2 {t in T, c in C diff C_loaded diff C_locked, p in P_load[c]}: qw[c,t,p] <= 1e5*x[c,t,p]; # link xB and wB
# qw > 0 ==> x1 = 1
subject to Condition112b2 {c in C, t in T, p in P}: qw[c,t,p] <= 1e5*x1[c,t,p]; # link x and qw 
# qw = 0 ==> x1 = 0
subject to Condition112b3 {c in C, t in T, p in P}: qw[c,t,p] >= x1[c,t,p]; # link x and qw
...
# banned ballast
subject to Condition114i1 {t in ballastBan, p in P_ban}: xB[t, p] = 0;
...
## DISCHARGE -----------------------------------------------------------------------
# min amount
subject to Condition120a {c in C, t in T, p in P_dis[c]}: -1e5*zw[c,t,p]  <= w[c,t,p];
subject to Condition120b {c in C, t in T, p in P_dis[c]}: w[c,t,p] <= -500*zw[c,t,p];

# min sounding # 4m sounding
subject to Condition121a {c in C, t in T, p in P_dis[c]}: sounding4mVol[t]*zqw[c,t,p]  <= qty[c,t,p]; 
subject to Condition121b {c in C, t in T, p in P_dis[c]}: qty[c,t,p] <= 1e5*zqw[c,t,p];

# for partial discharging
# non-empty until certain port
subject to Condition122c1 {c in C, t in T2[c], p in P_load2[c]}: qw[c,t,p] >= minCargoAmt; # other tank
subject to Condition122c2 {c in C, t in T3[c], p in P_load3[c]}: qty[c,t,p] >= vol30percent[t] - 0.1; # slop
subject to Condition122c3 {c in C, t in T3[c], p in P_load4[c]}: vol30percent[t] - 0.1 <=  qty[c,t,p] <= vol30percent[t] + 0.1; # slop
...
subject to Constr13 {p in P_stable0}: displacementLowLimit[p]+0.001 <= displacement[p] <= displacementLimit[p]-0.001;
