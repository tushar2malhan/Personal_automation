...
-1*sum{t in TB, p in P}wB[t,p] - 10000*sum{t in T} xt[t] +
...
## load all  cargo
subject to Condition111 {p in P_last_loading}: -intended <= min(cargoweight,sum{c in C, pp in P:Wcp[c,pp]>0}Wcp[c,pp]) - sum{t in T, c in C}qw[c,t,p]<=intended;
...
# amount ballast to adjust during dep and arr
