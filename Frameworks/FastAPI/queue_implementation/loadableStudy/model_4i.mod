...
set slopTankFirst within C cross T cross P default {};
...
param ave_trim {p in P} default 5.0; #0.5*trim_lower[p]+ 0.5*trim_upper[p];
...
-1*sum{t in TB, p in P}wB[t,p] - 10000*sum{c in C_loaded1, t in T, p in P_opt} qwz[c,t,p];
#####################
#original
...
subject to Condition03 {c in C diff C_locked}: sum{t in Tc[c]} Q0[c,t] + sum{p in P_load[c], t in Tc[c]} y[c,t,p] >= sum{p in P_dis[c], t in Tc[c]} -y[c,t,p]; # total loaded >= total discharged

subject to Condition04 {c in C diff C_locked, p in P_load[c] : Vcp[c,p]>0}:  0 <= sum{t in Tc[c]} y[c,t,p] <= Vcp[c,p]; # amount of cargo to be loaded <= amount available at the port.
subject to Condition041 {c in C, t in Tc[c], p in P_load[c]}: y[c,t,p] >= 0;
...
subject to Condition050 {c in C diff  C_locked, p in P_dis[c] : Vcp[c,p]<0}:  sum{t in Tc[c]}qty[c,t,p-1]>=-Vcp[c,p]-1e6*(1-delta[c,p]);
subject to Condition050a {c in C diff  C_locked, p in P_dis[c] : Vcp[c,p]<0}:  sum{t in Tc[c]}qty[c,t,p-1]<=-Vcp[c,p]+1e6*delta[c,p];
## if delta = 0,  -inf <= y[p] <= inf
## if delta = 1,   -Vcp[p] <= y[p] <= -Vcp[p]
subject to Condition050b {c in C diff  C_locked, p in P_dis[c] : Vcp[c,p]<0}:  -Vcp[c,p]-1e6*(1-delta[c,p])<=-sum{t in Tc[c]} y[c,t,p];
subject to Condition050b1 {c in C diff  C_locked, p in P_dis[c] : Vcp[c,p]<0}:  -sum{t in Tc[c]} y[c,t,p]<=-Vcp[c,p]+1e6*(1-delta[c,p]);
## if delta = 0,   0 <= qty[p] <= 0
## if delta = 1,   -inf <= qty[p] <= Inf
subject to Condition050c {c in C diff  C_locked, p in P_dis[c] : Vcp[c,p]<0}:  -1e6*(delta[c,p])<=-sum{t in Tc[c]} qty[c,t,p];
subject to Condition050c1 {c in C diff  C_locked, p in P_dis[c] : Vcp[c,p]<0}:  -sum{t in Tc[c]} qty[c,t,p]<=1e6*(delta[c,p]);

subject to Condition051 {c in C, t in Tc[c], p in P_dis[c]}: y[c,t,p]<=0;
...
subject to condition24b {c in C_loaded, t in T, p in fixCargoPort}: qty[c,t,p] = Q1[c,t,p]; # follow the existing stowage of preloaded cargoes
...
subject to Constr5a {c in C diff C_loaded diff C_locked, t in Tc[c], p in P_last_loading}: qty[c,t,p] <= upperBoundC[c,t]*capacityCargoTank[t]*x1[c,t,p];
# commingled
...
subject to Constr8 {c in C, t in Tc[c], p in P_last_loading}: qw2f[c,t,p]/1000 - 1e4*x1[c,t,p] <= 0; ## newly added on 20190603
...
subject to Constr12a1 {t in T}: sum {c in C, p in Pbar} x1[c,t,p] <= 1000*xt[t];
...
## DISCHARGING ----------------------------------------------------------------------------------
# slop tank first
subject to Condition123a {(c, t, p) in slopTankFirst}: qw[c,t,p] = 0; # 
## DISCHARGE -----------------------------------------------------------------------
...
subject to Condition122c2 {c in C, t in T3[c], p in P_load3[c]}: qw[c,t,p] >= vol30percent[t]; # slop
