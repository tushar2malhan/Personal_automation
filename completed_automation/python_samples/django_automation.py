...
def listdir_in_ur_home():
    return os.listdir(r'D:\Users\tusha\Desktop\django_prac\Django_bot')      # [ listing down your files in this home directory ]
def chdir_to_home():
    os.chdir(r'D:\Users\tusha\Desktop\django_prac\Django_bot')       # here ur projects and apps files will be installed  [ you can create ur directory and do changes accordingly ]

check_packages = os.listdir(r'd:\users\tusha\desktop\python\env\lib\site-packages')    # if django , rest_framework , packages are  installed or not , set path where ur packages are installed in ur local system 
main_dir =r'D:\Users\tusha\Desktop\django_prac\Django_bot'             # CREATE UR DIR FIRST 
...
if a in ['django' , 'go','','l','ok','yes']:
    ...
    try:
        ...
        if r:          
            ...
            print('issue in env2 and virtualenv')
        ...
        os.system(f'pip install django && pip install djangorestframework  in {check_packages}')
    ...
    if project_name in listdir_in_ur_home():
        ...
        print('Project with that initials already present Done now !  , You are good to go now')
            ...
            # os.system(f'python manage.py django startproject {project_name}')
    ...
    try:
        if 'yes' in db:
            ...
            if 'yes' in enter:
                    os.chdir(r'D:\Program Files\PostgreSQL\13\bin')                 # if u wanna enter shell directly kindly set ur path of Postgress/bin
                    time.sleep(2)
    ...
    # ask= input('Wanna add  modules in ur views.py ?\t ').strip().lower()
...
'''
    ...
    user_login= input('wanna create login portal for user ')
    if user_login in ['yes','yup','ok','y']:
